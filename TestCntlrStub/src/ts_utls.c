...
void tsReadImsiAndSendUeConfig(const char *ue_file, int noOfUe)
{
   ...
   while (fgets(temp, 100, fp) != NULL) {
      ...
      if (c != ' ')
      {
         printf("Error: invalid format reading IMSI on line %d\n", id);
         exit(1);
      }
      imsi_len = i;
      token_idx = i;
      ...
      if ((c != ' ') && (c != '\n'))
      {
         printf("Error: invalid format reading IMEI on line %d\n", id);
         exit(1);
      }
      token_idx += i;
      // skip spaces
      ...
      for (i = 0; i < 32 + 1; i++)
      {
         ...
         {
            printf("Error: invalid format reading negFlags on line %d\n", id);
            exit(1);
         }
      }
   finished:
...
int TC_msg_recv(int msgid, int timeout)
{
   msgids[0] = msgid;
   msgids[1] = 0;
   return TC_msg_recv_multi(msgids, NULL, timeout);
}

int TC_msg_recv_multi(int *succ, int *fail, int timeout)
{
   ...
   while (!msg_recvd)
   {
      ...
      tm.tv_sec += timeout;
      if (mq_timedreceive(msgQid, (void *)&msgptr, sizeof(MsgBuf), NULL, &tm) == -1)
      {
         printf("mq_receive failed with error\n");
         return 0;
      }
      if (succ) {
         index = 0;
         msgid = &succ[0];
         while (*msgid) {
            if ((int) msgptr.msgType == *msgid)
            {
               //printf("Received (msgType=%d) [ok]\n", *msgid);
               retVal = SUCC;
               msg_recvd = TRUE;
               goto msg_received;
            }
      ...
      if (fail) {
         ...
         while (*msgid) {
            if ((int) msgptr.msgType == *msgid)
            {
               //printf("Received (msgType=%d) [fail]\n", *msgid);
               retVal = FAIL;
               msg_recvd = TRUE;
               goto msg_received;
            }
      ...
      //printf("Received (msgType=%ld) [continue]\n", msgptr.msgType);
      retVal = FAIL;
   }
msg_received:
...
void TC_API(short int msgId, void *ptr, short int msgLen)
{
   ...
   clock_gettime(CLOCK_REALTIME, &tm);
   tm.tv_sec += 3;  // 3 second timeout here

   sbuf.msgType = msgId;
   sbuf.msgLen = msgLen;
   memcpy(sbuf.msg, ptr, msgLen);

   if (mq_timedsend(msgQid, (void *)&sbuf, sizeof(MsgBuf), 0, &tm) < 0)
   {
      printf("\nMessage send failed\n");
      perror("");
      exit(0);
...
void tsCompleteAttach(unsigned char ueId)
{
   ...
   */
   /* Wait for Attach Accept indication */
   int succ[] = {UE_ATTACH_ACCEPT_IND, 0};
   int fail[] = {UE_ATTACH_FAIL_IND, UE_ATTACH_REJECT_IND, 0};
   if (TC_msg_recv_multi(succ, fail, 3) == SUCC)
   {
      //printf("\n[Stub] Received Attach Accept indication\n");
   }
   else
   {
      printf("Attach failed for UE %d\n", ueId);
   }
}
...
void tsSendAttachRequest(unsigned char ueId)
{
   ...
   //printf("[Stub] Entering tsSendAttachRequest(), ueId(%d)\n", ueId);
...
void tsSendDetachRequest(unsigned char ueId, unsigned char cause, Ue_Mode ueMode)
{
   ...
   if (cause == UE_NORMAL_DETACH)
   {
      ...
      if (TC_msg_recv(UE_DETACH_ACCEPT_IND, 3) == SUCC)
      {
         //printf("\nReceived Detach Accept indication\n");
      }
   }
   if (TC_msg_recv(UE_CTX_REL_IND, 3) == SUCC)
   {
      //printf("\nReceived UE Context Release indication\n");
   }
}
