...
#ifdef MULTI_ENB_SUPPORT
   cfg->srcAddr.sctPort = osRand(); //(NBSM_LSZ_ENB_SCTPORT + suId);
   if (cfg->srcAddr.sctPort < 1024)
   {
      cfg->srcAddr.sctPort += 1024;
   }
#else
...
#endif /* ANSI */
{
   ...
   if (usta->alarm.event == LSZ_EVENT_LI_ASSOC_CFM && usta->alarm.cause == LSZ_CAUSE_ASSOC_UP)
   {
